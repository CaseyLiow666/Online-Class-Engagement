...
$feed = trim((string) ($_GET['feed'] ?? 'chat'));
...
$typeFilter = "m.message_type = 'text'";
if ($feed === 'announcements') {
    $typeFilter = "m.message_type IN ('announcement', 'notification')";
} elseif ($feed !== 'chat') {
    http_response_code(400);
    echo json_encode(['ok' => false, 'error' => 'Invalid feed']);
    exit;
}
...
$baseSql =
    "SELECT m.id, m.body, m.message_type, m.created_at, m.user_id, u.username, u.role AS user_role, u.full_name
     ...
     WHERE m.classroom_id = ? AND m.channel_id = ? AND {$typeFilter}";
...
echo json_encode([
    'ok' => true,
    'viewer_id' => $uid,
    'viewer_is_teacher' => $role === 'teacher',
    'messages' => $rows,
]);
